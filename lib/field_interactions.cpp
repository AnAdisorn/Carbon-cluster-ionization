...
std::array<Vector3d, 4> calculatePairFields(const std::string type1, const std::string type2, const Vector3d &r1, const Vector3d &r2, const Vector3d &v1, const Vector3d &v2)
{
    ...
    if (q1 * q2 < 0) // Soft-core coulomb law
    {
        if (q1 > 0 && !(IonisationParametersMap.find(lowerType(type1)) == IonisationParametersMap.end()))
        {
            double r_soft = pow(q1 * q2 / IonisationParametersMap[lowerType(type1)].kEi, 2);
            e2 = q1 / pow(r21.squaredNorm() + r_soft, 1.5) * r21;
            e1 = (-q2 / q1) * e2;
            return {e1, e2, b1, b2};
        }
        else if (q2 > 0 && !(IonisationParametersMap.find(lowerType(type2)) == IonisationParametersMap.end()))
        {
            double r_soft = pow(q1 * q2 / IonisationParametersMap[lowerType(type2)].kEi, 2);
            e1 = q2 / pow(r21.squaredNorm() + r_soft, 1.5) * (-r21);
            e2 = (-q1 / q2) * e1;
            return {e1, e2, b1, b2};
        }
    }
